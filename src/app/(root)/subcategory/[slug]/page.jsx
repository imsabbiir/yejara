    ...
    const fetchData = async () => {
      try {
        const productRes = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/api/subcategory/${slug}`,
          {
            cache: "no-store",
          },
        );
  ...
  return (
    <div className="px-6 py-10">
      ...
      <nav className="flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-[#5e534a] mb-6">
        ...
        <Link href={`/category/${category?.name}`} className="hover:text-coral">
          {category?.title}
        </Link>
        <ChevronRight className="w-3 h-3" />

        <Link href={`/subcategory/${slug}`} className="hover:text-coral">
      ...
      >
        <p className="text-xs font-mono uppercase tracking-[0.3em] text-[#17100b]/70 mb-3">
          {currentSubcategory?.title} · {filteredProducts.length} items
        </p>
